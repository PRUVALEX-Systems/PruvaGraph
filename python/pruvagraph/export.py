...
# ──────────────────────────────────────────────────────────────────────────────
# HTML visualisation (self-contained, D3 v7 from CDN)
# ──────────────────────────────────────────────────────────────────────────────
...
def _graph_to_d3(G: nx.MultiDiGraph) -> dict[str, Any]:
    ...
    for node_id, data in G.nodes(data=True):
        nodes.append({
            ...
            "color":     _COLOR_MAP.get(data.get("type", "unknown"), _COLOR_MAP["unknown"]),
        })
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  #canvas-wrap {{ position: relative; width: 100%; height: calc(100vh - 90px); }}
...
<div id="header">
  <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
    <circle cx="5" cy="5" r="3" fill="#7C6EFA"/>
    <circle cx="15" cy="5" r="3" fill="#22D3EE"/>
    <circle cx="10" cy="15" r="3" fill="#34D399"/>
    <line x1="5" y1="5" x2="15" y2="5" stroke="#7C6EFA" stroke-width="1.5"/>
    <line x1="5" y1="5" x2="10" y2="15" stroke="#7C6EFA" stroke-width="1.5"/>
    <line x1="15" y1="5" x2="10" y2="15" stroke="#22D3EE" stroke-width="1.5"/>
  </svg>
  <h1>PRUVALEX PruvaGraph</h1>
...
const linkSel = root.append('g').attr('class','links')
  ...
  .attr('class','link')
  .attr('stroke', d => d.relation === 'imports' ? '#30363D' : '#4B5563')
  .attr('stroke-width', 1);
...
function showTip(event, d) {{
  ...
  t.innerHTML = `
    <div class="t-label">${{d.label}}</div>
    <div class="t-type">${{d.type}} · community ${{d.community ?? 'N/A'}}</div>
    ${{d.summary ? `<div class="t-sum">${{d.summary}}</div>` : ''}}
    ${{d.file ? `<div class="t-file">${{d.file}}</div>` : ''}}
  `;
...
function searchNodes(q) {{
  ...
  nodeG.select('circle')
    .attr('opacity', d => !lq || d.label.toLowerCase().includes(lq) ||
      (d.summary||'').toLowerCase().includes(lq) ? 1 : 0.1);
  nodeG.select('text')
    .style('display', d => !lq
      ? ((degMap[d.id]||0) > 3 ? 'block' : 'none')
      : (d.label.toLowerCase().includes(lq) ? 'block' : 'none'));
}}
...
setTimeout(() => svg.call(zoom.transform, d3.zoomIdentity.translate(W()/2, H()/2).scale(0.8)), 100);
</script>
</body>
</html>"""
